...
async function isApprovedSite(url) {
  if (!url) return false
  const data = await chrome.storage.local.get(["approvedSites", "onboardingComplete"])
...
async function scanActiveTab() {
  try {
    ...
    if (!data.scanning || !data.onboardingComplete) {
      console.log("Scanning disabled or onboarding not complete")
      return
    }

    // Get the last focused window and its active tab
    ...
    if (!approved) {
      console.log("Not an approved site:", tab.url)
      return
    }
...
chrome.runtime.onMessage.addListener((msg, sender) => {
  if (["SCROLL_SETTLED", "PAGE_LOADED", "NEW_CONTENT_LOADED"].includes(msg.type)) {
    console.log("Received:", msg.type, "— triggering scan")
    scanActiveTab()
  }
})
